...
Star[] stars = new Star[Star.starCount]; //only variables is how many stars to draw
...
void draw() {
  ...
  //Must press the SPACEBAR to start Program
  if (startKeyboard==true) {
    for (int i = 0; i < stars.length; i++) {
      stars[i].step();
      ellipse(stars[i].getX(), stars[i].y, stars[i].raduis, stars[i].radius);
    }
  } //End of startStop
} //End draw()
